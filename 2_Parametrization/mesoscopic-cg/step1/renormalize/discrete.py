    ...
    if(k == 0 and line_element[0] != 'ITEM:'):  # Scan for all
        ...
        if ((time_element%1000 == 0)and (len(index_temp) == 2048)):  # Start the mapping process
            ...
            for i in range(0,2048):
                ...
                fs_val += phi[i]*phi[i]*phi[i]*(10-15*phi[i]+6*phi[i]*phi[i])*f_fs[int(phi[i]//0.05)]
                fw_cnt += phi[i]*phi[i]*(1-phi[i])*(1-phi[i])
                fw_val += phi[i]*phi[i]*(1-phi[i])*(1-phi[i])*f_fw[int(phi[i]//0.05)]
                fepsilon_cnt += density_grad[i]
                fepsilon_val += density_grad[i]*f_fepsilon[int(phi[i]//0.05)]
...
for i in range(0,len(fs_avg)):
    ...
    str_out = "%d %.6e %.6e %.6e\n" % (i+1,fs_val,fw_val,fepsilon_val)
    out_data.write(str_out)
...
str_out = "%.6e %.6e %.6e\n" %(fin_fs_val,fin_fw_val,fin_fepsilon_val)
print(str_out)
f.close()
fs.close()
#fl.close()
